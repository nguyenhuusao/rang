...
if( ! function_exists( 'sc_blog' ) )
{
	function sc_blog( $attr, $content = null )
	{
		...
		if( $filters && $style == 'masonry' && ! $category && ! $category_multi ){
			$output .= '<div id="Filters" class="isotope-filters" data-parent="column_blog">';
...
if( ! function_exists( 'sc_hover_box' ) )
{
	function sc_hover_box( $attr, $content = null )
	{
		...
		$output = '<div class="hover_box" ontouchstart="this.classList.toggle(\'hover\');">';
			if( $link ) $output .= '<a href="'. $link .'" '. $target .'>';
				$output .= '<div class="hover_box_wrapper">';
					$output .= '<img class="visible_photo scale-with-grid" src="'. $image .'" alt="" '. mfn_get_attachment_dimensions( mfn_get_attachment_id_url( $image ), 'width', 1 ) .' '. mfn_get_attachment_dimensions( mfn_get_attachment_id_url( $image ), 'height', 1 ) .'/>';
					$output .= '<img class="hidden_photo scale-with-grid" src="'. $image_hover .'" alt="" '. mfn_get_attachment_dimensions( mfn_get_attachment_id_url( $image ), 'width', 1 ) .' '. mfn_get_attachment_dimensions( mfn_get_attachment_id_url( $image ), 'height', 1 ) .'/>';
				$output .= '</div>';
			if( $link ) $output .= '</a>';
		$output .= '</div>'."\n";
...
if( ! function_exists( 'sc_portfolio' ) )
{
	function sc_portfolio( $attr, $content = null )
	{
		...
		if( $filters && ! $category && ! $category_multi ){
			$output .= '<div id="Filters" class="isotope-filters" data-parent="column_portfolio">';
				$output .= '<div class="filters_wrapper">';
					$output .= '<ul class="categories">';
